...
- GPU memory snapshots are enabled for every LoRA shared-base worker (`{gpu}_{image}_lora`), including B200/B300. One lifecycle serves every model: start vLLM with `--enable-sleep-mode`, require a successful warmup, then `POST /sleep?level=1` to preserve base weights in CPU memory and discard the KV cache. Restore runs `POST /wake_up`; tenant adapters are attached only afterward. There is no checkpoint reload, model-name allowlist, or change to the capacity-based GPU selector. Snapshot workers reserve host RAM by GPU capacity for the offloaded weights. `modal_shared.serving.snapshot.SnapshotExecutor` uses file-backed rendezvous to avoid TCPStore sockets broken by restoration; the deployed stacks are single-GPU and multi-rank configurations are rejected. Warmup, sleep, and wake failures prevent inference; `@enter` records errors instead of raising because Modal otherwise retries failed enter hooks indefinitely. Snapshot logs include a preserved origin and a fresh runtime ID to distinguish snapshot reuse from construction. Full-FT/merged workers remain unsnapshotted. `pre_warm` boots and verifies the pool before deployment becomes READY; Modal may still create additional hardware-profile snapshots. Snapshots are per base and serving configuration, never per adapter. Snap workers enable `VLLM_SERVER_DEV_MODE=1` for the internal sleep/wake endpoints. Their compilation cache stays container-local: a mutable shared compilation Volume can lose files referenced by a snapshot and fail restoration before Python runs. Plain workers reload/commit `overmind-vllm-cache`. Weight loading uses `runai_streamer`; serving images install `runai-model-streamer>=0.15.7`. CUDA graphs use `CUDAGRAPH_CAPTURE_SIZES`; changes require rechecking `ACTIVATION_OVERHEAD_GB` in `gpu_selector`. Completions and playground SSE send idle pings every 15s to keep the 60s edge ALB timeout from cutting off cold boots; non-stream completions stream JSON and encode late failures in the response body.
