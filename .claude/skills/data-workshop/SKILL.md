...
- `services/datasets/use.py` is the one gate every consumer (eval runs, finetuning jobs, optimizer experiments) passes. `use.check(dataset, intent, cell=)` checks the dataset's intent and both contracts on the version and changes nothing: validators, readiness tools and estimates call it. `use.use` / `use.freeze` set `used_at` and run only where the consumer's row is created, so a refused launch never freezes a version. The FK PROTECTs the cell. A finetuning job pins `cell`, `validation_cell` and `eval_cell`; every later read (overlap, context guard, in-training evals) uses the pinned cells, never `active_cell`. A used cell and every cell before it are frozen (`Cell.frozen`), the intent and capability with them.
