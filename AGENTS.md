...
- Datasets are a source and a chain of cells; versions are derived, never stored; `use.use(dataset, intent, cell=)` is the one gate every consumer passes and it freezes the version; the chain, name, intent and capability change only through the dataset's own chat. Detail: data-workshop skill.
- Tenancy is project-scoped, no org layer. A guest holds one project and can only read and claim; a view that accepts guest writes opts in with `guest_allowed = True` — never a permission class or middleware. API keys are `scope=account` or `scope=project`. Detail: backend-architecture § Auth and tenancy.
...
- Icons come from the central registry only: `import { Icon } from "@/components/ui/icons"`. The glyphs are vendored in `ui/icons/pixelart.tsx`; never import that module or `lucide-react` in app code — `check:design` fails on both.
