...
- Datasets are a source and a chain of cells; versions are derived, never stored; `use.use(dataset, intent, cell=)` is the one gate every consumer passes and it freezes the version; the Console changes the chain, name, intent and capability through the dataset's own chat; REST `PATCH` and the cell endpoints reach the same `lifecycle` functions. Detail: data-workshop skill.
