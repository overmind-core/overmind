...
function VersionStatusBadge({ status }: { status?: string }) {
  ...
  const map: Record<string, { label: string; className: string }> = {
    pending:    { label: "Pending",    className: "border-amber-400/60 text-amber-700 dark:text-amber-400" },
    superseded: { label: "Superseded", className: "text-muted-foreground" },
    rejected:   { label: "Rejected",   className: "border-red-400/60 text-red-600 dark:text-red-400" },
  };
