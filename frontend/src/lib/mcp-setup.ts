...
export function telemetrySetupPrompt(client: McpClient): string {
  ...
  return [
    ...
    "Follow `references/telemetry.md`. Call get_instrumentation_plan with no capability for project-wide work or the supplied capability for scoped work. If it returns human_action or no placements, report the instruction and stop. Refresh setup, sync, and the plan when a non-empty version_analyzed_sha does not match the current checkout; stop if it remains stale.",
    "Apply every placement as an exact ticket. When delegating, compute its files from target.file and required_spans[].target.file, coalesce overlapping tickets under one owner, and never let two workers edit the same file. Preserve each capability_id, key, required_scope, required_spans, identity, grain, target, version, and fingerprint verbatim.",
    "After applying the tickets, report the changed files and checks, generate a unique verification correlation, and ask the user to choose Real run (recommended) or Smoke run. Present the exact command or input, capability, environment, provider/model, expected side effects, correlation value, and approved attempt count for each choice; mark unknown fields as needing user input. Do not run either mode before explicit approval.",
