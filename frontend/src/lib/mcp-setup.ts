...
export function telemetrySetupPrompt(client: McpClient): string {
  ...
  return [
    ...
    "Follow `references/telemetry.md`. Call `get_instrumentation_plan` with no capability for project-wide work or the supplied capability for scoped work. If it returns `human_action` or no placements, report the instruction and stop this attempt. Compare each non-empty `version_analyzed_sha` with the current checkout; refresh setup, sync, and the plan when stale, then stop if it remains stale.",
