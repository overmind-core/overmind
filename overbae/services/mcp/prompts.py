...
PROMPTS = (
    ...
    PromptDefinition(
        ...
        template=(
            ...
            "instruction and stop. Before editing, compare each non-empty version_analyzed_sha with "
            "the current checkout; refresh setup, sync, and the plan when stale, then stop if it still "
            "does not match. Preserve every ticket field verbatim. When delegating, compute each "
            "ticket's files from target.file and required_spans[].target.file, and give overlapping "
            "tickets one owner so two workers never edit the same file. The MCP server does not edit "
            "the repository or ingest traces. After applying the changes, report the tickets, changed "
            "files, and local checks. Generate a unique verification correlation value, then ask the "
            "user to choose Real run (recommended) or Smoke run. For each choice, present the exact "
            "command or input, capability, environment, provider/model, expected side effects, "
            "correlation value, and approved attempt count; mark unknown fields as needing user input. "
            "Do not run either mode before approval. Stamp the approved correlation as conversation.id "
            "with the application's existing mechanism or overmind.set_conversation_id, run only the "
            "approved input, and flush spans. Poll query_traces(session=<correlation>, "
            "all_spans=false, limit=2) within a fixed bound and require page.total == 1. Read that "
            "row's overmind://traces/{{trace_id}} resource, require truncated == false and "
            "span_count == len(spans), then pass the supplied spans unchanged to "
            "verify_instrumentation. Report application outcome separately from instrumentation "
            "status. A real-run retry needs fresh approval unless the user approved an exact input "
