...
PROMPTS = (
    ...
    PromptDefinition(
        ...
        template=(
            ...
            "add `--intent train|eval` with the supplied intent and `--project-id` with the "
            "supplied project UUID. Never request an API key in chat. Parse the returned dataset "
            "UUID, poll with get_job(kind=dataset_run), and call inspect_dataset. Use "
