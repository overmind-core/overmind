...
A backend is a thin wrapper around :class:`overclaw.optimize.runner.AgentRunner`
...
* :class:`ShadowBackend` — runs the agent in a subprocess with every
  ...
  iterating on agents that cannot be run end-to-end on OverClaw's
  infrastructure.
...
from overclaw.optimize.cassette import Cassette, open_cassette
...
from overclaw.optimize.runner import AgentRunner, RunOutput
...
class BackendOutput:
    ...
    Wraps :class:`RunOutput` with OverClaw-specific metadata so the optimizer
