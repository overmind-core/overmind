...
async def get_agent_detail(
    ...
    # Determine active version and pending version
    active_prompt = next(
        (v for v in all_versions if v.status == PROMPT_STATUS_ACTIVE), None
    )
    max_prompt = all_versions[0]  # already ordered by version desc

    # Auto-accept: if a pending version has real production spans, activate it
    if active_prompt and max_prompt.status == PROMPT_STATUS_PENDING:
...
async def accept_prompt_version(
    ...
    Atomically flips status: the requested version becomes active and
    all other versions of the same slug+project become superseded.  If there is
    a pending suggestion that targets this version it is also marked as accepted.
    """
    if project_id:
        pid = _uuid.UUID(project_id)
    elif user.user.projects:
    ...
    for v in all_versions:
        if v.version != target.version and v.status != "rejected":
            v.status = PROMPT_STATUS_SUPERSEDED
