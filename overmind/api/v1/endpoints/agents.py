...
from overmind.api.v1.endpoints.jobs import JobOut
...
async def list_agents(
    ...
    # Only active versions — one per slug, guaranteed by DISTINCT ON
    prompts = await get_active_prompts_for_project(pid, db)

    # Fetch the latest pending version per slug in a single query (for the "vN available" badge)
    pending_version_by_slug = await get_pending_version_by_slug(pid, db)
    ...
    for prompt in prompts:
        ...
        agents.append(
            AgentOut(
                ...
                latest_version=pending_version_by_slug.get(prompt.slug),
                analytics=analytics,
...
class PromptVersionOut(BaseModel):
    ...
    status: str = PROMPT_STATUS_ACTIVE
...
class AgentDetailOut(BaseModel):
    ...
    name: str
    project_id: str
    # Deprecated: identical to active_version. Kept for backward-compatibility
    # with older frontend/SDK versions that still read this field.
    latest_version: int
    active_version: int
...
async def get_agent_detail(
    ...
    # Determine active and pending versions.
    # Auto-accept (when a pending version accumulates real spans) is handled
    # exclusively by the _auto_accept_pending_versions Celery task so that GET
    # requests remain side-effect-free and safe to retry/cache.
    active_prompt = next(
        (v for v in all_versions if v.status == PROMPT_STATUS_ACTIVE), None
    )
    max_prompt = all_versions[0]  # already ordered by version desc

    # Fall back to max_prompt if no active row found (legacy data safety)
    ...
    pending_version = (
        ...
        and max_prompt.version != active_prompt.version
        else None
    )
...
# ---------------------------------------------------------------------------
# Reject a specific prompt version
# ---------------------------------------------------------------------------
...
class RejectVersionResponse(BaseModel):
    slug: str
    rejected_version: int


@router.post("/{prompt_slug}/reject-version", response_model=RejectVersionResponse)
async def reject_prompt_version(
    ...
    """
    Reject a specific prompt version, keeping the current active version unchanged.

    Marks the requested version as ``rejected`` and dismisses any pending
    suggestion that targets it.  Rejected versions are excluded from future
    pipeline runs and displayed accordingly in the UI.
    """
    if project_id:
        pid = uuid.UUID(project_id)
    elif user.user.projects:
    ...
    if target.status == PROMPT_STATUS_ACTIVE:
        raise HTTPException(
            status_code=400,
            detail="Cannot reject the currently active version",
        )

    target.status = PROMPT_STATUS_REJECTED

    # Dismiss any pending suggestion that targets this version
    pending_sugg_q = await db.execute(
        select(Suggestion).where(
            and_(
                Suggestion.prompt_slug == prompt_slug,
    ...
    return RejectVersionResponse(slug=prompt_slug, rejected_version=request.version)
