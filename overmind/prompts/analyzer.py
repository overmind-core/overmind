"""Prompts for ``overmind.optimize.analyzer``.
...
Overmind invokes `{entrypoint_fn}(input)` from `{entry_file}` \
...
Overmind calls `{entrypoint_fn}(input)` from `{entry_file}` (input is a dict; return a dict). \
