...
1. **Ticketed instrumentation.** Call `get_instrumentation_plan` with no
   ...
   no placements, report its instruction and stop. Copy `target.file`,
   `qualname`, required scope, required decorator, import line, and capability
   id. `lineno` and `source_line` are often null — file + qualname is enough to
   locate the function. Refresh setup, sync, and the plan when a non-empty
   `version_analyzed_sha` differs from the current checkout; stop if the fresh
   ticket still differs. Keep a primary scope outermost when a specialized span
   targets the same function. When coding subagents are available and permitted,
   group tickets by every file they touch, including
   `required_spans[].target.file`, so one worker owns each overlapping group and
   two workers never edit the same file.
1. **Explicit run approval.** After applying the ticketed code changes, report
   the changed files and any local checks, generate a unique verification
