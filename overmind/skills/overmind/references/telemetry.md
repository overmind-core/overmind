...
registry returns an explicit human action; report it and stop. Do not continue
when `placements` is empty. Run local `/overmind setup` and `overmind sync`,
then request the plan again.
...
owner so two workers never edit the same file. The parent integrates the
groups and owns all verification.
