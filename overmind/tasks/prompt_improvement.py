...
"""
    ...
    logger.info(
        f"Generated {len(suggestions)} improvement suggestions (agentic_behavior={has_agentic_spans})"
    )
    return suggestions
    ...
    async def _generate_one(old_span: SpanModel) -> dict[str, Any]:
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Could not format prompt with input variables for span {old_span.span_id}: missing key {e}"
                )
