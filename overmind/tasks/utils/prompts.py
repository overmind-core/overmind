...
CORRECTNESS_SYSTEM_PROMPT = """Return ONLY valid JSON. Always include 'correctness' (float 0.0–1.0; 0 means completely incorrect, 1 means fully correct). When correctness < 0.5, also include 'reason' (string, 1–2 sentences explaining the main issues). When correctness >= 0.5, omit 'reason' entirely."""
...
AGENTIC_CORRECTNESS_SYSTEM_PROMPT = """Return ONLY valid JSON. Always include 'correctness' (float 0.0–1.0; 0 means completely incorrect, 1 means fully correct). When correctness < 0.5, also include 'reason' (string, 1–2 sentences explaining the main issues with the final output). When correctness >= 0.5, omit 'reason' entirely."""
...
TOOL_CALL_CORRECTNESS_SYSTEM_PROMPT = """Return ONLY valid JSON. Always include 'correctness' (float 0.0–1.0; 0 means completely wrong tool selection or arguments, 1 means perfect tool selection and well-formed arguments). When correctness < 0.5, also include 'reason' (string, 1–2 sentences explaining the main issues with the tool selection or arguments). When correctness >= 0.5, omit 'reason' entirely."""
...
TOOL_ANSWER_CORRECTNESS_SYSTEM_PROMPT = """Return ONLY valid JSON. Always include 'correctness' (float 0.0–1.0; 0 means completely incorrect, 1 means fully correct and faithful to the tool results). When correctness < 0.5, also include 'reason' (string, 1–2 sentences explaining where the answer diverges from or misuses the tool results). When correctness >= 0.5, omit 'reason' entirely."""
