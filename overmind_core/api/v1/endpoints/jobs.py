...
async def create_job_from_user(
    ...
    if data.job_type == JobType.MODEL_BACKTESTING:
        from overmind_core.tasks.backtesting import (
    ...
    if data.job_type == JobType.PROMPT_TUNING:
        from overmind_core.tasks.prompt_improvement import validate_prompt_tuning_eligibility
...
async def trigger_job(
    ...
    from overmind_core.tasks.prompt_improvement import improve_prompt_templates
    from overmind_core.tasks.auto_evaluation import auto_evaluate_unscored_spans_task
    from overmind_core.tasks.agent_discovery import run_agent_discovery_task
    ...
    if job.job_type == JobType.MODEL_BACKTESTING.value:
        from overmind_core.tasks.backtesting import run_model_backtesting_task
